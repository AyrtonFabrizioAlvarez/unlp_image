...
import UNLPimage.src.functions.files_functions as files_function
...
def collage(images_number, images_names):
    ...
    components_left = [
        [
            sg.T(f"Foto {elem + 1}", font=FONT_BODY),
            sg.Combo(
                images_names,
                key=f"-IMAGE-{elem}-",
                size=(15, 5),
...
def run(pattern_name):
    """Funcion que genera la iteracion de la ventana de collage, recibe como
    parametro el nombre del diseño de patron para la generar la ventana,
    captando eventos de los diferentes elementos que la componen,
    posibilita la creacion de la image, su modificacion y su guardado
    en memoria, en caso de guardar la imagen crea una entrada en el log
    y almacena los datos del evento.
    ...
    try:
        ...
        collage_paths = files_function.open_record()
        names = []
        title = ""
        while True:
            ...
            match event:
                ...
                case "-SAVE-":
                    ...
                    if answer == "Yes":
                        files_function.election(
                            image, title, collage_paths["-COLLAGEPATH-"]
                        )
                        if title == "":
                            title = "default"
